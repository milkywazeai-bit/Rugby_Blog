...
from train_alert.fake import AVAILABLE, SOLD_OUT, FakeTrain
from train_alert.matcher import SEAT, alert_key, build_result
...
def train(dep_time, general=SOLD_OUT, special=SOLD_OUT, standby="0", number="301"):
    return FakeTrain(
        train_number=number,
        dep_date="20260924",
        dep_time=dep_time,
        ...
        dep_station_name="수서",
        arr_station_name="부산",
        general_seat_state=general,
        special_seat_state=special,
        reserve_wait_possible_code=standby,
    )
...
class AvailabilityTest(unittest.TestCase):
    ...
    def test_seat_wins_over_standby(self):
        trains = [train("061000", general=AVAILABLE, standby="9")]
...
class ConfigFileTest(unittest.TestCase):
    ...
    def test_station_names_exist_on_the_srt_network(self):
        try:
